...
$SSH_GW ip address add 10.0.0.254/24 dev veth_ns1
$SSH_GW ip netns exec ns1 ip address add 10.0.0.1/24 dev veth1
$SSH_GW ip netns exec ns1 ip route add $NETMASK_VIRTUAL via 10.0.0.254

# Manual ARP resolution
MAC_GW="$( $SSH_GW ip -brief link show veth_ns1 | awk '{ print $3 }' )"
$SSH_GW ip netns exec ns1 arp -s 10.0.0.254 $MAC_GW
MAC_CLIENT="$( $SSH_GW ip netns exec ns1 ip -brief link show veth1 |\
               awk '{ print $3 }' )"
$SSH_GW arp -s 10.0.0.1 $MAC_CLIENT
if ! $SSH_GW ip netns exec ns1 ping -c 10 $IPADDR_VIRTUAL &>/dev/null ; then
